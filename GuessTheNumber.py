...
# of the numbers the computer is allowed to choose from and the amount of attempts the player
# has to guess the right number is decided.
# The player is promoted to choose a number and is told if it is higher or lower then the
# computer's number. If the number is the same number the computer chose, the player has won.
# If the player can't guess the number within the allow attempts, he loses.
#
...
class Game:
    ...
    # Sets the highest number the computer can choose.
    ...
    # Chooses a random number between 0 and the highest allowed number generated by setMaxRange method
...
and you have to guess it (otherwise you DIE MUHAHAHA)\n""")
    ...
    # Sets the difficutly according to the player choice
    ...
    # Validtion method - checks whether or not the number entered by the player is within range
    ...
    # Checks whether the input is higher, lower then or equals to the computer's number
    def checkInput(self, input):
        self.attemptsTried += 1
        if(input == self.chosenNumber):
            print("That is current! You win!")
            return True                                             # Returns True to stop the game
